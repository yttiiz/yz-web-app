...
export class AuthController extends DefaultController {
    ...
    #loginRouteHandler = async <T extends AuthPathType>(ctx: RouterContextAppType<T>) => {
    ...
    #registerRouteHandler = async <T extends AuthPathType>(ctx: RouterContextAppType<T>) => {        
        let photo: string;
        const data = await ctx.request.body().value as oak.FormDataReader;
        ...
        const { fields: {
            ...
            birth,
            job
        }, files } = await data.read({ maxSize: 10_000_000 });
    ...
    #fileHandler(
        ...
        .map(async (file) => {
            if (file.filename) {
                ...
                photo = `img/users/${firstname.toLowerCase()}_${lastname.toLowerCase()}.${ext}`;
